...
本プロジェクトでは、プロンプトの変更履歴を体系的に管理するため、**スナップショット**戦略を採用しています。

**スナップショットディレクトリ**: `prompt/snap-shot/`

- **目的**: プロンプト更新時に変更前の状態をスナップショットとして保管し、バージョン管理を実現
- **保管対象**: すべての主要プロンプト（医療記録生成、ミーティング記録、退院前訪問指導など）
- **命名規則**: `{機能名}-{バージョン番号}.md`（例: `pt-1.0.md`, `ot-2.1.md`）

#### スナップショット保管ルール

1. **作成タイミング**
   - プロンプトに重大な変更を加える直前
   - 重大な変更とは：機能仕様の変更、ルール・制約の修正、出力形式の変更など
   - 軽微な修正（誤字訂正、ドメイン知識の更新など）のみの場合は不要

2. **-latest ファイルの役割**
   - `-latest` 接尾辞が付与されたプロンプト（例: `pt-prompt-latest.md`）が**変更履歴を保存する最新版**
   - スナップショットは過去の安定版を参照する際に使用
   - 常に最新の本番環境用プロンプトは `-latest` で管理

3. **バージョンナンバリング**
   - セマンティックバージョニングを採用: `{メジャー}.{マイナー}`
   - 重大な機能変更: メジャー版を増分
   - 機能追加・改善: マイナー版を増分

4. **スナップショット保管プロセス**
   ```
   1. 更新対象プロンプトの現在バージョンを確認（例: v1.5）
   2. 次のバージョンを決定（例: v2.0の場合、現在のv1.5を保存）
   3. 現在のプロンプトを snap-shot/ ディレクトリにコピー
   4. ファイル名を {機能名}-{現在バージョン}.md に変更（例: pt-1.5.md）
   5. -latest ファイルに新しい内容を反映
   6. git コミット時に両方のファイルをコミット
   ```

#### スナップショット参照方法

プロンプトの変更に問題が生じた場合、スナップショットから以前のバージョンを参照できます：
...
# 特定バージョンを確認
cat prompt/snap-shot/pt-1.0.md
```
